...
dataWF <-
    workflow() %>%
    add_model(rfModelLogReg) %>%
    add_recipe(dataRecipe)

rfModelLogRegFit <- dataWF %>%
    fit(data = trainData)

rfTrainingPred <-
    predict(rfModelLogRegFit, trainData) %>%
    bind_cols(predict(rfModelLogRegFit, trainData, type = "prob")) %>%
    bind_cols(trainData %>%
                  select(Survived))
...
#  Testing Data
rfTestingPred <-
    predict(rfModelLogRegFit, testData) %>%
    bind_cols(predict(rfModelLogRegFit, testData, type = "prob")) %>%
    bind_cols(testData %>%
                  select(Survived))
...
rfTestingPred <-
    predict(rfModelLogRegFit, finalTestData)
